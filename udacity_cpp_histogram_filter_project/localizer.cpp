	...
	float p_miss)
{
	...
	for (size_t y = 0; y < grid.size(); y++)
	{
		...
		{
			float p = (color == grid[y][x]) ? p_hit : p_miss;
			temp_beliefs.push_back(beliefs[y][x] * p);
		}
	...
	float blurring)
{
	...
	// your code here
	int height = static_cast<int>(beliefs.size());
	int width = static_cast<int>(beliefs[0].size());
	int new_i;
	int new_j;
