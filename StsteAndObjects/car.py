...
class Car(object):
    ...
    def move(self, dt=1):
        ...
        # Predict the new position [y, x] based on velocity [vx, vy] and time,
        # dt
        predicted_position = [(position[0] + velocity[0] * dt) % height, # default dt = 1
    ...
    def turn_left(self):
        ...
        # Change the velocity self.state = [position, velocity]
        velocity = self.state[1]
    ...
    def display_world(self):
        ...
        if(len(self.path) > 1):
            # loop through all path indices and draw a dot (unless it's at the
            # car's location)
